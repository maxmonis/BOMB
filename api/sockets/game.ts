...
import { getAvailableGames } from "../ws"

export async function gameSocket(
  ws: Socket,
  {
    games,
    gameSockets,
  ...
  let player = userId ? game?.players.find(p => p.id == userId) : null
  ...
  ws.on("message", async rawData => {
    ...
    try {
      // -------------------- Accept Join Request --------------------
      if (req.key == "accept_join_request") {
        ...
        sendGameState(game, gameSockets)
      }
      ...
      // -------------------- Leave Game --------------------
        ...
        sendToast(game, gameSockets, `${player.name} left the game`)
...
function sendGameState(game: Game, gameSockets: Map<string, Socket>) {
